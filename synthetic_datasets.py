...
def generate_cross_dataset(
    bar_width,
    bar_height,
    offset,  # vertical offset of horizontal bar center from vertical bar center
    n_samples_vert=500,
    n_samples_horiz=500,
    random_state=None
):
    """
    Generate a dataset with two perpendicular bars in a T or + configuration.

    Parameters:
    - bar_width: float
        Width of each bar (perpendicular to length).
    - bar_height: float
        Length of each bar (long dimension).
    - offset: float
        Vertical offset of the *horizontal* bar center from vertical bar center.
        0.0 forms a +; bar_height / 2 forms a T.
    - n_samples_vert: int
        Number of samples for vertical bar.
    - n_samples_horiz: int
        Number of samples for horizontal bar.
    - random_state: int or None
        Seed for reproducibility.

    Returns:
    - X: np.ndarray of shape (n_samples_vert + n_samples_horiz, 2)
    - y: np.ndarray of shape (n_samples_vert + n_samples_horiz,)
    """
    # assert 0 <= bar_width <= 1.0 and 0 <= bar_height <= 1.0, "Bar dimensions must be in [0, 1]"
    # assert offset >= 0, "Offset must be non-negative"

    rng = np.random.default_rng(random_state)
    cx, cy = 0.5, 0.5

    # Center of vertical bar: fixed at (0.5, 0.5)
    x_vert = rng.uniform(cx - bar_width / 2, cx + bar_width / 2, n_samples_vert)
    y_vert = rng.uniform(cy - bar_height / 2, cy + bar_height / 2, n_samples_vert)
    X_vert = np.stack((x_vert, y_vert), axis=1)

    # Center of horizontal bar: (0.5, 0.5 + offset)
    cy_horiz = cy + offset
    # if not (0 <= cy_horiz - bar_width / 2 and cy_horiz + bar_width / 2 <= 1.0):
    #     raise ValueError("Horizontal bar exceeds vertical bounds.")
    # if not (0 <= cx - bar_height / 2 and cx + bar_height / 2 <= 1.0):
    #     raise ValueError("Horizontal bar exceeds horizontal bounds.")

    x_horiz = rng.uniform(cx - bar_height / 2, cx + bar_height / 2, n_samples_horiz)
    y_horiz = rng.uniform(cy_horiz - bar_width / 2, cy_horiz + bar_width / 2, n_samples_horiz)
    X_horiz = np.stack((x_horiz, y_horiz), axis=1)

    y_vert = np.zeros(n_samples_vert, dtype=int)
    y_horiz = np.ones(n_samples_horiz, dtype=int)

    X = np.vstack((X_vert, X_horiz))
    y = np.concatenate((y_vert, y_horiz))
