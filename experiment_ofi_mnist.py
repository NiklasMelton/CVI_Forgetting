...
def experiment_ofi_mnist_cnn(seed = 42):
    ...
    X_train_all = train_ds.data.unsqueeze(1).float() / 255.0
    ...
    for order in ("Shuffled", "Ordered"):
        ...
        print(f"→ Running MNIST condition: {order}")
    ...
    print("Saved MNIST CNN traces with OFI, Overlap Index, and per-batch accuracy.")
...
def experiment_ofi_mnist_knn(seed = 42):
    torch.manual_seed(seed)
    np.random.seed(seed)
    ...
    print("Saved MNIST KNN traces with OFI, Overlap Index, and per-batch accuracy.")
