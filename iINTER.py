import numpy as np
from artlib import FuzzyART, HypersphereART, SimpleARTMAP
...
class iINTER:
    def __init__(
        ...
        if ART == "Fuzzy":
            module_a = FuzzyART(rho=rho, alpha=1e-10, beta=1.0)
        else:
            module_a = HypersphereART(rho=rho, alpha=1e-10, beta=1.0, r_hat=r_hat)

        self.ARTMAP = SimpleARTMAP(module_a)
        self.sparse_adj = defaultdict(lambda: 0)
        self.cluster_cardinality = GrowingArray1D()
        self.rev_map = defaultdict(set)
        self.pairwise_index = defaultdict(lambda: 0.0)
        self.singleton_index = defaultdict(lambda: 0.0)
        self.index = 1.0
    ...
    def add_sample(self, x, y):
        ...
        if update_flag:
            self.singleton_index[y] = 1.0-max(
    ...
    def add_batch(self, X, Y):
        ...
        update_flags = defaultdict(lambda: False)
        for x, y, bmu1 in zip(X, Y, BMU1):
            self.rev_map[y].add(bmu1)
