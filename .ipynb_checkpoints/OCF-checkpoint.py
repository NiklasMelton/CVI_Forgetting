from OverlapIndex import OverlapIndex
import numpy as np
from typing import Literal, Optional, Dict, Tuple
from collections import defaultdict

def detect_catastrophic_forgetting_from_labels(
    y_true_prev: np.ndarray,
    y_pred_prev: np.ndarray,
    y_true_curr: np.ndarray,
    y_pred_curr: np.ndarray,
    oi_obj: Optional[OverlapIndex] = None,
    drop_threshold: float = 0.3,
    overlap_threshold: float = 0.9
) -> Dict[str, float]:
    """
    Detect catastrophic forgetting with optional refinement via OverlapIndex overlap indices.

    Returns
    -------
    Dict[str, float] with:
        'max_drop': max recall drop
        'drop_due_to_overlap': whether high singleton index coincided with drop
    """
    y_true_prev = np.asarray(y_true_prev, dtype=int)
    y_pred_prev = np.asarray(y_pred_prev, dtype=int)
    y_true_curr = np.asarray(y_true_curr, dtype=int)
    y_pred_curr = np.asarray(y_pred_curr, dtype=int)

    shared_classes = set(np.unique(y_true_prev)) & set(np.unique(y_true_curr))
    if not shared_classes:
        return {"max_drop": 0.0, "drop_due_to_overlap": False}

    def recall(y_true, y_pred):
        counts = defaultdict(int)
        correct = defaultdict(int)
        for yt, yp in zip(y_true, y_pred):
            counts[yt] += 1
            if yt == yp:
                correct[yt] += 1
        return {c: correct[c] / counts[c] for c in counts if counts[c] > 0}

    r_prev = recall(y_true_prev, y_pred_prev)
    r_curr = recall(y_true_curr, y_pred_curr)

    max_drop = 0.0
    overlap_caused = False

    for cls in shared_classes:
        drop = r_prev.get(cls, 0.0) - r_curr.get(cls, 0.0)
        if drop > max_drop:
            max_drop = drop
            if oi_obj is not None:
                singleton_score = oi_obj.singleton_index.get(cls, 0.0)
                if singleton_score > overlap_threshold:
                    overlap_caused = True

    return {
        "max_drop": max_drop,
        "drop_due_to_overlap": overlap_caused
    }



class OCF:
    def __init__(
            self,
            oi_thresh: float = 0.9,
            acc_drop_thresh: float = 0.3,
            rho: float = 0.9,
            r_hat: float = np.inf,
            ART: Literal["Fuzzy", "Hypersphere"] = "Hypersphere",
            match_tracking = "MT+",
    ):
        self.oi_thresh = oi_thresh
        self.acc_drop_thresh = acc_drop_thresh
        self.iInter = OverlapIndex(rho=rho, r_hat=r_hat, ART=ART, match_tracking=match_tracking)
        self.state = set()
        self.oi_indices = []
        self.acc_drops = []

    
    def add_batch(
        self,
        X_train: np.ndarray,
        y_train: np.ndarray,
        y_pred_eval: np.ndarray,
        y_true_eval: np.ndarray,
        
    ) -> str:
        """
        Update the OCF with a training batch and evaluation result.
    
        Returns the current *event* label for this batch.
        Persistent conditions are stored in `self.state` (a set).
        """
        # 1. Compute oi index from training batch
        oi_index = self.iInter.add_batch(X_train, y_train)
        self.oi_indices.append(oi_index)
    
        # 2. Helper to compute per-class recall
        def compute_recall(y_true, y_pred):
            counts = defaultdict(int)
            correct = defaultdict(int)
            for yt, yp in zip(y_true, y_pred):
                counts[yt] += 1
                if yt == yp:
                    correct[yt] += 1
            return {c: correct[c] / counts[c] for c in counts if counts[c] > 0}
    
        # 3. Compute recall and update max recall per class
        curr_recalls = compute_recall(y_true_eval, y_pred_eval)
    
        if not hasattr(self, "max_recalls"):
            self.max_recalls: Dict[int, float] = {}
    
        max_drop = 0.0
        for cls, curr_r in curr_recalls.items():
            prev_best = self.max_recalls.get(cls, 0.0)
            self.max_recalls[cls] = max(prev_best, curr_r)
            drop = prev_best - curr_r
            max_drop = max(max_drop, drop)
    
        self.acc_drops.append(max_drop)
    
        # 4. Instantaneous forgetting/overlap detection
        is_forgetting = max_drop > self.acc_drop_thresh
        is_overlap = oi_index < self.oi_thresh

        if is_overlap:
            self.event = "overlap"
        elif is_forgetting:
            self.event = "forgetting"
        else:
            self.event = "none"
    
        # 5. Update persistent state (add/remove)
        if is_forgetting:
            self.state.add("CF")
        else:
            self.state.discard("CF")
    
        if is_overlap:
            self.state.add("overlap")
        else:
            self.state.discard("overlap")
    
        return self.state


def compute_recall(y_true, y_pred):
    counts = defaultdict(int)
    correct = defaultdict(int)
    for yt, yp in zip(y_true, y_pred):
        counts[yt] += 1
        if yt == yp:
            correct[yt] += 1
    return {c: correct[c] / counts[c] for c in counts}

def compute_mean_scores(scores, y_true):
    mean_scores = defaultdict(float)
    for c in range(scores.shape[1]):
        class_scores = scores[:, c]
        true_scores = class_scores[y_true==c]
        mean_scores[c] = float(np.mean(true_scores))
    return mean_scores


class OCF2:
    def __init__(
            self,
            rho: float = 0.9,
            r_hat: float = np.inf,
            ART: Literal["Fuzzy", "Hypersphere"] = "Hypersphere",
            match_tracking = "MT+",
    ):
        self.OI = OverlapIndex(rho=rho, r_hat=r_hat, ART=ART, match_tracking=match_tracking)
        self.global_indices = {"overshadowing": 0.0, "forgetting": 0.0}
        self.cluster_indices = defaultdict(lambda: {"overshadowing": 0.0, "forgetting": 0.0})
        self.max_scores: Dict[int, float] = defaultdict(lambda: -np.inf)
        self.min_scores: Dict[int, float] = defaultdict(lambda: np.inf)
        self.max_recalls: Dict[int, float] = defaultdict(float)


    def add_batch(
        self,
        X_train: np.ndarray,
        y_train: np.ndarray,
        y_pred_eval: np.ndarray,
        y_true_eval: np.ndarray,
        y_scores_eval: np.ndarray

    ) -> Tuple[float, float]:
        """
        Update the OCF with a training batch and evaluation result.

        (overshadowing, forgetting, residual) as floats in [0, 1]
        """
        # 0. check if we lost any classes in the evaluation set
        historical = set(self.max_recalls.keys())
        current = set(int(x) for x in y_true_eval)  # ensure plain ints
        missing = historical - current
        assert not missing, f"Missing classes in eval: {missing}"

        # 1. Compute oi index from training batch
        _ = self.OI.add_batch(X_train, y_train)


        # 2. Compute recall and score changes per class
        curr_recalls = compute_recall(y_true_eval, y_pred_eval)
        mean_scores = compute_mean_scores(y_scores_eval, y_true_eval)

        recall_drops = defaultdict(float)
        score_drops = defaultdict(float)

        for cls, curr_r in curr_recalls.items():
            prev_best = self.max_recalls[cls]
            self.max_recalls[cls] = max(prev_best, curr_r)
            recall_drops[cls] = self.max_recalls[cls] - curr_r

            # allow for rebalancing if recall isn't degraded
            if curr_r >= prev_best:
                self.max_scores[cls] = mean_scores[cls]
                if np.isinf(self.min_scores[cls]):
                    self.min_scores[cls] = mean_scores[cls]
            else:
                self.max_scores[cls] = max(self.max_scores[cls], mean_scores[cls])
                self.min_scores[cls] = min(self.min_scores[cls], mean_scores[cls])
            score_span = self.max_scores[cls] - self.min_scores[cls]
            if score_span > 0:
                score_drops[cls] = (
                    self.max_scores[cls] - mean_scores[cls]
                )/score_span
            else:
                score_drops[cls] = 0.0


        # 3. compute indices
        Fs: Dict[int, float] = {c: recall_drops[c]*score_drops[c] for c in recall_drops}


        Os: Dict[int, float] = {
            c: recall_drops[c]*(
                (1.-self.OI.singleton_index[c])
            )*(1-score_drops[c])
            for c in recall_drops.keys()
        }
        for c in recall_drops:
            FO_sum = Fs[c]+Os[c]
            if FO_sum > 0:
                ratio = recall_drops[c]/FO_sum
                Fs[c] = ratio*Fs[c]
                Os[c] = ratio*Os[c]

        F = float(np.mean(list(Fs.values())))
        O = float(np.mean(list(Os.values())))

        for c in curr_recalls.keys():
            self.cluster_indices[c]["overshadowing"] = Os[c]
            self.cluster_indices[c]["forgetting"] = Fs[c]

        self.global_indices["overshadowing"] = O
        self.global_indices["forgetting"] = F

        return O, F





    
